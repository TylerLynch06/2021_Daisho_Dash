...
Playerfalling - Same as above, just for falling
'''
##Animation
'''
...
Frametracker - There is a 'self.frametracker' in every sprite that dicates animati, every frame the sprite is updated, this accumulates (it resets to -1 (-1 becuase its added to at the start of every update) when it reaches the FPS
...
'''
The Player:
The Player class is very volatile, one tiny chnage may ruin everything, so before modifying, save as!
'''
...
Boss_movecount = 0
Boss_hp = 2
##Time UNTIL Boss, not length of boss fight
##This is in seconds
Boss_time = 30
...
def Player_reset():
    ...
    player.rollframes = 0
    player.FIGHTREADY = False
    player.sheathing = False
        ...
        if Gamebegun == True:
            ...
            while True:
                try:
                    ...
                    if Ready == False and Boss_incoming == False:
...
class RocketPlatform(pygame.sprite.Sprite):

    def __init__(self,x,y):
        pygame.sprite.Sprite.__init__(self)
        self.image = Floater[0]
        self.rect = self.image.get_rect()
        self.rect.centerx = x
        self.rect.top = y
        self.frametracker = 0
    ...
    def update(self):
        ...
        if self.xoffset>10 and self.offsetup == True: 
        elif self.offsetup == True: self.offsetup == False
        elif selrf.xoffset>-10 and sel
        else: self.xoffset-=1
        self.frametracker+=1
        if self.frametracker>FPS:
            self.frametracker = 0
...
rocketplatform = RocketPlatform(XMiddle,HEIGHT-500)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == PLAYERJUMP_EVENT:
    ...
    try:
        if RS.reset == True:
            ...
            ##Killing Rocketplatform, as it is also different
            try:
                enemylist = pygame.sprite.Group.sprites(rocketplat)
                RP = enemylist[0]
                RP.kill()
    ...
    if hits:
        if BD.dead == True:
            ...
            if player.dead == False:
                if player.rect.centerx<BD.rect.centerx+30 and player.rect.centerx>BD.rect.centerx-30:
                    Gore_slash.play()
                    player.dead = True
