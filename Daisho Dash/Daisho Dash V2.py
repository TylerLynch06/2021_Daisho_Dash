...
##NOTES:
##True game start is when large plat is in correct Y position
...
class Hatman(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        try:
            if self.frametracker >3 and player.dead == False and self.slashing == False:
                
                self.frame = HMSprint[self.anitrack]
                self.anitrack +=1
                self.frametracker = -1
            elif self.frametracker > 2 and self.slashing == True:
...
while running:
    ...
    if player.dead == False and Ready == True:
        ...
        ##Teaman spawn Timer
        if Enemieson == True:
            teaman_timer = pygame.time.get_ticks() - now6 - UntilStart
        ...
        if Enemieson == True:
            barondown_timer = pygame.time.get_ticks() - now7 - UntilStart
    ...
    if hits:
        if SP.dead == True:
            ...
            if SP.rect.left>player.rect.centerx-50:
                SP.dead = True
                Sword_clash.play()
    ...
    if Gamebegun == True:
        ...
        ##Key Rendering
    ##Platforms always in front (sparing HUD)
